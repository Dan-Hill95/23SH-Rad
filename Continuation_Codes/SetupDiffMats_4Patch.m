...
r(1) = 1; R = sparse(1:N,[1:N],1./r,N,N); r(1) = 0;
...
for i=1:4
LM(1+(i-1)*N:i*N,1:N) = D2 + R*D + e - (i*m)^2*(R.^2).*e; % d_rr + d_r/r + I-(i*m/r)^2
LM(1+(i-1)*N,1) = (2-((m*i)^2)/2)*D2(1,1) + 1; LM(1+(i-1)*N,2) = (2-((m*i)^2)/2)*D2(1,2); % 2d_rr at r= 0
LM(1+(i-1)*N:i*N,1:N) = -LM(1+(i-1)*N:i*N,1:N)^2; % -(1+laplacian)^2
end
